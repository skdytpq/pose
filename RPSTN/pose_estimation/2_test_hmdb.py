...
class Trainer(object):
    ...
    def validation(self, epoch):
        ...
        for i, (input, heatmap, label, img_path, bbox, start_index) in enumerate(tbar):
            ...
            if self.is_visual:
                heat_file_name = 'result/JHMDB/heats/{}_batch_update.jpg'.format(i)
                heat_file_name2 = 'result/JHMDB/heats/{}_batch_pred.jpg'.format(i)
                coor_file_name = 'result/JHMDB/coords/gt_pred_batch_{}_update.jpg'.format(i)
                ...
                gt_val, gt_maxval = evaluate.get_max_preds(heatmap_var.view(-1, self.numClasses, heat.shape[-2], heat.shape[-1]).detach().cpu().numpy())
                gt_label = label[:, :, :, :-1].view(-1, self.numClasses, 2)
                vis = label[:, :, :, -1].view(-1, self.numClasses, 1)


                # np.save('result/JHMDB/coords/preds.npy', pred_val)
                # np.save('result/JHMDB/coords/label.npy', gt_label)
                # np.save('result/JHMDB/coords/gt.npy', gt_val)
                # np.save('result/JHMDB/coords/vis.npy', vis)
                # np.save('result/JHMDB/imgs/input_img.npy', input)
                # pdb.set_trace()

            input = input.view(b, t, c, h, w)
            heat = heat.view(b, t, self.numClasses, heat.shape[-2], heat.shape[-1])
